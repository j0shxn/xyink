...
args_cell = argv();
args = str2double(cell2mat(args_cell));
disp(args);

class(str2double(args))
...
matlabResult = Matlab_xy(args(1), args(2))
xyinkResult = Xyink(args(1), args(2), 0)
cnt = 0;
...
for azdArr = 1:355
    for eldArr = 1:89
        cnt++;
        matlabResult = Matlab_xy(azdArr, eldArr);
        xyinkResult = Xyink(azdArr, eldArr, 0);
        matlabXHist(cnt) = matlabResult(1);
        matlabYHist(cnt) = matlabResult(2);
        xyinkXHist(cnt) = xyinkResult(1);
        xyinkYHist(cnt) = xyinkResult(2);
    end
end

tim = 1:cnt;
plot(tim, matlabXHist - xyinkXHist)
pause
%}

%{

For some reason the system thinks its out of workspace in the azimuth 91 to
-181 to 360
270 band 
%}
