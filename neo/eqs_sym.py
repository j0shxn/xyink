...
def cart2sph(cV):
    ...
    cVR = sqrt(cV.dot(cV))
    cVu = cV / cVR
    cVA = atan2(cVu[0], cVu[1])
    cVB = asin(cVu[2])
    sphV = Matrix([cVA, cVB, cVR])
    return sphV
...
x_eq = x_eq.simplify()
print("Transcendental PSI equation:")
pprint(x_eq)
...
# We take the negative of it because of -PSI at the start.
#psi_ceq_0 = psi_comp_eq[0].simplify()
...
print("\nPSI_C1 = \n")
pretty_print(psi_ceq_1) # THIS IS THE RIGHT ONE
...
PHI = phi_eq
F = sqrt( PHI ** 2 + PSI ** 2 )
g = sqrt( diff(F, alpha)**2 + diff(F,beta)**2 )
#G = g.simplify()
print(g.free_symbols)

g0 = g.subs(zeta, pi/2-0.1)
g1 = g0.subs(gamma, pi/2-0.1)
g2 = g1.subs(alpha, 1.250635029342023)
g3 = g2.subs(beta, -0.9504808490870142)
G = g3.evalf()
print(G)
